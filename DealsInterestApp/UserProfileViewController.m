...
//  DealsInterestApp
//
//  Created by Mervin Tan Kok Wei on 29/10/13.
...
- (void)LoadView
{
    ...
    _profileSlideOut.frame = CGRectMake(self.view.frame.size.width, _profileSlideOut.frame.origin.y, 0, _profileSlideOut.frame.size.height);
...
-(IBAction)onOpenButtonClick:(id)sender
{
    ...
    if (_profileSlideOut.frame.origin.x == self.view.frame.size.width/2)
    {
        
        _profileSlideOut.frame = CGRectMake(self.view.frame.size.width, _profileSlideOut.frame.origin.y, 0, _profileSlideOut.frame.size.height);
        _profileSlideBtn.frame = CGRectMake((self.view.frame.size.width - _profileSlideBtn.frame.size.width), _profileSlideBtn.frame.origin.y, _profileSlideBtn.frame.size.width, _profileSlideBtn.frame.size.height);
        
    }
    else
    {
        _profileSlideOut.frame = CGRectMake(self.view.frame.size.width/2, _profileSlideOut.frame.origin.y, self.view.frame.size.width/2, _profileSlideOut.frame.size.height);
        _profileSlideBtn.frame = CGRectMake((_profileSlideOut.frame.size.width - _profileSlideBtn.frame.size.width), _profileSlideBtn.frame.origin.y, _profileSlideBtn.frame.size.width, _profileSlideBtn.frame.size.height);
    }
...
- (void)getUserStatistics{
    ...
    NSString *postData = [NSString stringWithFormat:@"user_id=%@", postUserId];
...
- (void)getJsonObject{
    ...
    //initialize url that is going to be fetched.
    AppDelegate *appDelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    NSLog(@"%@",[NSString stringWithFormat:@"%1$@json/user.getUserById/%2$@",appDelegate.apiUrl,postUserId]);
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%1$@json/user.getUserById/%2$@",appDelegate.apiUrl,postUserId]];
